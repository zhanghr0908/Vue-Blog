...
export default {
  ...
  methods: {
    getData() {
      const _this = this
      this.$axios.get('/visitLogList?currentPage=' + this.queryInfo.pageNum+"&pageSize=" + this.queryInfo.pageSize,{
        headers: {
    ...
    deleteLogById(id) {
      const _this = this
      this.$axios.get('/visitLog/delete/'+id).then((res) => {
        _this.$alert('操作成功', '提示', {
    ...
    search() {
      ...
      const _this = this
      this.$axios.get('/visitLog/part?uuid='+this.queryInfo.uuid+'&time='+query.date+'&currentPage='+ query.pageNum+"&pageSize=" + query.pageSize,{
        headers: {
          "Authorization": localStorage.getItem("token")
        }
      }).then((res) => {
